...
import { Calendar, Clock, Phone, Users, ChevronDown, RefreshCcw } from "lucide-react"
...
export default function EmpleadoReservacionesPage() {
  ...
  const todayReservations = useMemo(() => {
    ...
    return reservations.filter(r => r.date.split("T")[0] === today)
  }, [reservations])

  const getUpcomingReservations = useMemo(() => {
    const now = new Date()
    return reservations
      .filter(r => new Date(r.date + "T" + r.hour) > now)
  ...
  return (
    ...
    >
      <div className="space-y-8">
        <section>
          <h1 className="font-serif text-3xl text-foreground md:text-4xl">Reservaciones</h1>
          <p className="mt-2 text-sm text-muted-foreground">Visualiza y gestiona las reservas de los clientes.</p>
        </section>
        ...
        {!loading && !error && (
          <>
            {/* Próximas Reservaciones - Widget */}
            {getUpcomingReservations.length > 0 && (
              <section className="rounded-2xl border bg-card p-6">
                <h2 className="font-serif text-2xl text-foreground mb-4">Próximas Reservaciones</h2>
                <div className="space-y-3">
                  {getUpcomingReservations.map((reservation) => (
                    <div key={reservation.id} className="flex items-center justify-between rounded-lg bg-background/50 p-4">
            ...
            {todayReservations.length > 0 && (
              <section className="rounded-2xl border bg-card p-6">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="font-serif text-2xl text-foreground">Horarios en Uso - Hoy</h2>
                </div>
                <div className="grid gap-2 sm:grid-cols-6 lg:grid-cols-12">
                  {todayReservations
                    .sort((a, b) => a.hour.localeCompare(b.hour))
                    .map((reservation) => (
                      <div key={reservation.id} className="rounded-lg border border-border bg-background p-3 text-center">
                        <div className="text-sm font-bold text-foreground">{reservation.hour}</div>
                        <div className="text-xs text-muted-foreground truncate">{reservation.name}</div>
                        <div className="text-xs font-medium">{reservation.guests} personas</div>
                      </div>
                    ))}
                </div>
              </section>
                          ...
                          {dateReservations
                            ...
                            .map((reservation) => (
                              <tr key={reservation.id} className="border-b last:border-0 hover:bg-background/50">
                                ...
                                <td className="px-6 py-4 text-sm font-medium">{reservation.name}</td>
